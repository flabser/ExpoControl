...
public class BaseDocument implements Const, Serializable {
    ...
    public String dbID;
    public HashMap<String, BlobField> blobFieldsMap = new HashMap<>();
    public ArrayList<BaseDocument> responses = new ArrayList<>();
    ...
    public ArrayList<BigDecimal> viewNumberList = new ArrayList<>();
    public ArrayList<Date> viewDateList = new ArrayList<>();
    ...
    protected HashSet<Reader> readers = new HashSet<>();
    protected HashSet<String> editors = new HashSet<>();
    protected String authorID = "undefined";
    protected String currentUserID;
    protected String sign;
    protected String signedFields;
    protected ArrayList<String> viewTextList = new ArrayList<>();
    ...
    public void getAttachments(String fieldName, String toPutFolder) {
        ...
        try {
            ...
            ResultSet attachResultSet = attachStatement.executeQuery("select * from CUSTOM_BLOBS_MAINDOCS " +
                    " where CUSTOM_BLOBS_MAINDOCS.DOCID = "
                    + docID +
                    " AND CUSTOM_BLOBS_MAINDOCS.NAME = '"
                    + fieldName + "' ");
    ...
    public void fillBlobs(HashMap<String, BlobField> blobs) {
        /* remove entries from existing fieldsMap that have not been received from blobFieldsMap first
		 * blobFieldsMap here is like a table (blobs) there
		 * and blobs here is like a blobFieldsMap there
		 */
        blobs_loop:
        for (Entry<String, BlobField> blob : this.blobFieldsMap.entrySet()) {
            files_loop:
	...
	/* now add files that are absent in blobFieldsMap */
        blobs_loop:
        for (Entry<String, BlobField> blob : blobs.entrySet()) {
            //files_loop:
            for (BlobFile file : blob.getValue().getFiles()) {
                if (!blobFieldsMap.containsKey(blob.getKey())) {
                    blobFieldsMap.put(blob.getKey(), blob.getValue());
                    continue blobs_loop;
                }
                BlobField existingBlob = blobFieldsMap.get(blob.getKey());
                existingBlob.addFile(file);
				/*for (BlobFile file2: blobFieldsMap.get(blob.getKey()).getFiles()) {
						if (file2.equals(file)) {
							if (!file2.originalName.equals(file.originalName)) {
								file2.originalName = file.originalName;
								continue files_loop;
							}
						}
					}
					if (!blobFieldsMap.get(blob.getKey()).hasFileName(file.originalName)) {
    ...
    public String[] getValueAsString(String fieldName) throws DocumentException {
        ...
        if (field != null) {
            if (field.getType() == FieldType.TEXTLIST || field.getType() == FieldType.VECTOR) {
                int ms = field.valuesAsStringList.size();
                if (ms > 0) {
                    String[] res = (String[]) field.valuesAsStringList.toArray(new String[ms]);
                    return res;
                } else {
            ...
            if (blobField != null) {
                ...
                String[] res = (String[]) blobField.getFileNames().toArray(new String[ms]);
                return res;
            } else {
    ...
    public Object getValueAsObject(String fieldName) throws DocumentException {
        ...
        if (field != null) {
            if (field.getType() == FieldType.COMPLEX_OBJECT) {
    ...
    public void setAccessRelatedFields(kz.flabs.runtimeobj.document.BaseDocument doc, HashMap<String, SaveFieldRule> saveFieldsMap, HashMap<String, String[]> fields) throws WebFormValueException {
        ...
        for (ISaveField saveField : saveFieldsMap.values()) {
            switch (saveField.getSourceType()) {
                ...
                case WEBFORMFILE:
                    ...
                    String filesToDelete[] = (String[]) fields.get("delete" + saveField.getName() + "name");
                    if (filesToDelete != null) {
                        for (String fileName : filesToDelete) {
                            try {
                                String originalAttachName1 = new String(((String) fileName).getBytes("ISO-8859-1"), "windows-1251");
                                String attachmentName = new String(((String) originalAttachName1).getBytes("cp1251"), "UTF-8");
                    ...
                    if (uploadedFiles.size() == 0) continue;
                    ...
                    //				String commentField = ((String[])fields.get(SiteFileUtil.getCommentFieldName(originalName, fullName, size)))[0];
    ...
    private Field getWebFormValue(ISaveField saveField, Map<String, String[]> fields) throws WebFormValueException {
        try {
            String valueToSave[] = (String[]) fields.get(saveField.getValue());
            ...
            if (saveField.getIfErrorValue().equals("")) {
                //throw new WebFormValueException(WebFormValueExceptionType.FORMDATA_INCORRECT, saveField.getValue());
    ...
    public ArrayList<BaseDocument> getDescendantsArray(int docID, int docType, Set<String> complexUserID, String absoluteUserID) throws DocumentException, ComplexObjectException {
    ...
    public ArrayList<String> copyAttachments(BaseDocument targetDoc) {
        ...
        for (BlobField existingBlob : this.blobFieldsMap.values()) {
            for (BlobFile existingFile : existingBlob.getFiles()) {
                existingFile.path = this.db.getDocumentAttach(this.getDocID(), this.docType, sysGroupAsSet, existingBlob.name, existingFile.originalName);
    ...
    public void parseXml(org.w3c.dom.Document xmlDoc, int pDocId) throws ComplexObjectException, IllegalAccessException, InstantiationException, ClassNotFoundException, _Exception {
                ...
                default:
                    switch(XMLUtil.getTextContent(k, "@type")){
                        ...
                        case "number":
                            ...
                            break;
                        case "complex":
                            Class c = Class.forName(k.getTextContent().substring(0, k.getTextContent().indexOf("~")));
                            IComplexObject obj = (IComplexObject)c.newInstance();
                            obj.init(db, k.getTextContent());
...
//        String viewText = getViewText();
        ...
        for (int i = 0; i < viewTextList.length; i++) {
            viewTexts += " viewtext" + i + "=\"" + (
                    viewTextList[i] != null && viewTextList[i].trim().length() > 0 ?
                            viewTextList[i].trim().replace("&amp;", "&").replace("&lt;", "<").replace("&gt;", ">").replace("&quot;", "\"").replace("&apos;", "'")
        ...
        for (Field field : fields()) {
            String value =  field.getType() == FieldType.COMPLEX_OBJECT ? field.valueAsObject.getPersistentValue() : field.valueAsText;
            value = value.replace("&amp;", "&").replace("&lt;", "<").replace("&gt;", ">").replace("&quot;", "\"").replace("&apos;", "'").replace("&nbsp;", " ");
            value = value.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\"", "&quot;").replace("'", "&apos;");
            xmlFragment.append("<" + field.name + RuntimeObjUtil.getTypeAttribute(field.getTypeAsDatabaseType()) + ">" + value
                    + "</" + field.name + ">");
        }
...
//        }
        ...
        if (allDescendants == false) {
            ...
            xml += " doctype = \"" + docType + "\" form= \"" + form + "\" dbid= \"" + dbID + "\" docid = \"" + docID + "\" " +
                    ...
                    " author=\"" + authorID + "\" ddbid = \"" + ddbID + "\" " +
                    "regdate=\"" + Util.convertDataTimeToString(regDate) + "\" " +
            ...
            xml += " hasDescendant=\"" + hasDescendant + "\" id=\"" + ddbID + "\" lastupdate=\"" + Util.convertDataTimeToString(lastUpdate)
                    + "\" viewtext =\"" + (viewTextList[0] != null && viewTextList[0].trim().length() > 0 ?
                            viewTextList[0].trim().replace("&amp;", "&").replace("&lt;", "<").replace("&gt;", ">").replace("&quot;", "\"").replace("&apos;", "'")
                                    .replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\"", "&quot;").replace("'", "&apos;") : "null" ) + "\">" + "<fields>" + xmlFragment + "</fields>";
    ...
    public String toShortXML() {
        ...
        String xml = "<document doctype = \"" + docType + "\"" +
                "id = \"" + ddbID + "\" viewtext=\"" + viewText + "\" " +
	...
	public String getParentDocumentID() {
		try {
			BaseDocument parentDoc = db.getDocumentByComplexID(parentDocType, parentDocID, Const.supervisorGroupAsSet, Const.sysUser);
