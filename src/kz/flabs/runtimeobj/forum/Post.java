...
public class Post extends Document {
	...
	public int save(User user) throws DocumentAccessException, DocumentException{
		int docID = 0;
	/*	setViewText();
		this.computeViewText();*/
        normalizeViewTexts();
		if(isNewDoc()){
			setRegDate(new Date());
			setLastUpdate(getRegDate());
			docID = db.getForum().insertPost(this, user.getAllUserGroups(), user.getUserID());
			setDocID(docID);
			setNewDoc(false);
		}else{
			setLastUpdate(new Date());
			docID = db.getForum().updatePost(this, user.getAllUserGroups(),  user.getUserID());
		}
		return docID;
	}
