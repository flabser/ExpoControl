...
public class Login extends HttpServlet implements Const {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if (env == null || env.isSystem){
			...
			}else{AppEnv.logger.warningLogEntry("Authorization failed, special login or password is incorrect");	
				...
				if (qID == null){
					...
					if (user.authorized){
						...
						ua.postLogin(userSession.browserType, ServletUtil.getClientIpAddr(request), userID);
