...
public class Provider extends HttpServlet implements Const{	
	...
	protected void  doPost(HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if (env != null){
				if (id != null){
					...
					if(type.equals("json")){				
						...
						new PortalException("Request has been undefined, type=" + type + ", id=" + id	+ ", key=" + key, env, response, ProviderExceptionType.PROVIDERERROR, PublishAsType.HTML, userSession.skin);
						return;
					}
					...
					if(result.publishAs == PublishAsType.JSON){
						...
						ProviderOutput po = new ProviderOutput(type, id, result.output, request, response, userSession, jses, result.title, result.addHistory);
					...
					//		System.out.println(type + " " + Util.getTimeDiffInMilSec(start_time));
				}else{
			...
			if (Environment.workspaceAuth){
				...
				if (jses == null) jses = request.getSession(true);
	...
	private ProviderResult json(UserSession userSession, String id) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException{
	...
	private ProviderResult content(HttpServletRequest request, HttpServletResponse response, IRule rule, UserSession userSession, String id) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException{
	...
	private ProviderResult outline(HttpServletRequest request, IRule rule, UserSession userSession, String id) throws UnsupportedEncodingException, RuleException, QueryFormulaParserException, DocumentException{
		...
		if (subType.equals("search")){
			...
			//outline = new DocumentOutline(env, id, key, page, userSession, type).getOutlineAsXML((String) jses.getAttribute("lang"));
	...
	@Deprecated
	private ProviderResult view(HttpServletRequest request, HttpServletResponse response, IRule rule, UserSession userSession, String id) throws ViewException, DocumentException, DocumentAccessException, RuleException, QueryFormulaParserException, QueryException, LocalizatorException, ComplexObjectException{
		...
		if (commandURL != null && (!commandURL.equals("null"))){
			...
			for(String command: commands){
				try{
					...
					new PortalException("Command has not recognized, incorrect syntax of the command=" + command, env, response, ProviderExceptionType.PROVIDERERROR, PublishAsType.HTML, userSession.skin);							
					//return;
				}
			}
		}	
		...
		result.output.append(view.getContent(fields, page, pageSize, parentDocProp[0], parentDocProp[1],  userSession.expandedThread, userSession.expandedCategory, userSession.getFlashDoc()));
	...
	private ProviderResult page(HttpServletResponse response, HttpServletRequest request, IRule rule, UserSession userSession) throws DocumentException, FTIndexEngineException, RuleException, QueryFormulaParserException, IOException, DocumentAccessException, QueryException, ClassNotFoundException{
		...
		fields.put("json", new String[]{sb.toString()});

		Page page = new Page(env, userSession, pageRule);	
		//result.output.append(page.process(fields, request, response, id, userSession, jses));
	...
	private ProviderResult query(HttpServletRequest request, IRule rule, UserSession userSession) throws DocumentException, FTIndexEngineException, RuleException, QueryFormulaParserException, UnsupportedEncodingException, DocumentAccessException, QueryException{
		ProviderResult result = new ProviderResult();
		result.addHistory = false;
		int parentDocProp[] = getParentDocProp(request);
		HashMap<String, String[]> fields = new HashMap<String, String[]>();
	...
	private ProviderResult count(HttpServletRequest request, IRule rule, UserSession userSession) throws DocumentException, FTIndexEngineException, RuleException, QueryFormulaParserException, UnsupportedEncodingException, DocumentAccessException, QueryException{
		ProviderResult result = new ProviderResult();	
		HashMap<String, String[]> fields = new HashMap<String, String[]>();
		Map<String, String[]> parMap = request.getParameterMap();	
		fields.putAll(parMap);		
	...
	private ProviderResult search(HttpServletRequest request, UserSession userSession) throws DocumentException, FTIndexEngineException, RuleException, QueryFormulaParserException, UnsupportedEncodingException, ComplexObjectException{
		...
		String keyWord = request.getParameter("keyword");
		keyWord = new String(((String)keyWord).getBytes("ISO-8859-1"),"UTF-8");
		FTSearchRequest ftRequest = new FTSearchRequest(env, userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID(), keyWord, page, userSession.pageSize);
		result.output.append(ftRequest.getDataAsXML());
		result.addHistory = true;
		return result;
	}

	private ProviderResult edit(HttpServletRequest request, IRule rule, UserSession userSession, String key) throws RuleException, DocumentException, DocumentAccessException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, ComplexObjectException{		
		...
		if (key == ""){						
			result.output.append(form.getDefaultFieldsAsXML(parentDocProp[0], parentDocProp[1], page, userSession.lang) + "<formsesid>" + Util.generateRandomAsText("0qwertyuiopasdfghjklzxcvbnm123456789") + "</formsesid>");
		}else{	
			if (element.equalsIgnoreCase("document")) {	
				...
				if (docID != null){
					...
					doc = db.getMainDocumentByID(Integer.parseInt(key), userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
				}
			}else if (element.equalsIgnoreCase("project")) {	
				doc = db.getProjects().getProjectByID(Integer.parseInt(key), userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
			}else if (element.equalsIgnoreCase("task")) {	
				doc = db.getTasks().getTaskByID(Integer.parseInt(key), userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
				...
				doc = glos.getGlossaryDocumentByID(Integer.parseInt(key), false, userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
			...
			if (doc.isValid){
				result.output.append(form.getFormAsXML(doc, page, doc.parentDocID, doc.parentDocType, userSession.lang) + "<formsesid>" + Util.generateRandomAsText("0qwertyuiopasdfghjklzxcvbnm123456789") + "</formsesid>");
			}else{
	 ...
	 * @deprecated**/
	private ProviderResult document(HttpServletRequest request, HttpServletResponse response, IRule rule, UserSession userSession, String key, String id, String type) throws DocumentAccessException, RuleException, QueryFormulaParserException, DocumentException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, NumberFormatException, ComplexObjectException{
		//FormRule formRule = (FormRule) rule;
		...
		if (key == ""){						
			...
			if (type.equals("glossary")){
				...
				if(id.equalsIgnoreCase("kr") || id.equalsIgnoreCase("comment")){
					...
					doc = db.getExecutions().getExecutionByID(Integer.parseInt(key), userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());	
				}else if (id.equalsIgnoreCase("project") || id.equals("outdocprj") || id.equalsIgnoreCase("workdocprj") || id.equalsIgnoreCase("remark")){
					doc = db.getProjects().getProjectByID(Integer.parseInt(key), userSession.currentUser.getAllUserGroups(), userSession.currentUser.getUserID());
	...
	private ProviderResult structure(HttpServletRequest request, HttpServletResponse response, IRule rule, UserSession userSession, String key, String id) throws DocumentAccessException, DocumentException, RuleException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, ComplexObjectException{
		//FormRule formRule = (FormRule)rule;
		FormRule formRule = (FormRule)env.ruleProvider.getRule(FORM_RULE, id);
		ProviderResult result = new ProviderResult(formRule.publishAs, formRule.getXSLT());	
		int page = 0;		
	...
	private ProviderResult forum(HttpServletRequest request, HttpServletResponse response, IRule rule, UserSession userSession, String key, String id) throws DocumentAccessException, DocumentException, RuleException, QueryFormulaParserException, QueryException, LocalizatorException, ClassNotFoundException, _Exception, ComplexObjectException{
		FormRule formRule = (FormRule)rule;
		//FormRule formRule = (FormRule)env.ruleProvider.getRule(FORM_RULE, id);
		ProviderResult result = new ProviderResult(formRule.publishAs, formRule.getXSLT());	
		int page = 0;		
	...
	private ProviderResult save(HttpServletRequest request, HttpServletResponse response, IRule rule, UserSession userSession, String key) throws UnsupportedEncodingException, WebFormValueException, LicenseException, DocumentException, RuleException, QueryFormulaParserException, DocumentAccessException, ClassNotFoundException, ComplexObjectException{
		ProviderResult result = new ProviderResult();	
	...
	private ProviderResult getAttach(HttpServletRequest request, UserSession userSession, String key) throws UnsupportedEncodingException{
		...
		String reqEnc = request.getCharacterEncoding();
		result.originalAttachName = new String(((String)attachName).getBytes("ISO-8859-1"),reqEnc);

		String formSesID = request.getParameter("formsesid");	
		if (formSesID != null){
			File file = Util.getExistFile(result.originalAttachName, Environment.tmpDir + File.separator + formSesID + File.separator + fieldName + File.separator);
			if (!file.exists()){					
				IDatabase dataBase = env.getDataBase();	
				String docTypeAsText = request.getParameter("doctype");
				//result.filePath = dataBase.getDocumentAttach(Integer.parseInt(key), Integer.parseInt(docTypeAsText), userSession.currentUser.getAllUserGroups(), fieldName, result.originalAttachName);
				result.filePath = dataBase.getDocumentAttach(Integer.parseInt(key), Integer.parseInt(docTypeAsText), fieldName, result.originalAttachName);
			}else{
	...
	private ProviderResult delete(HttpServletRequest request, UserSession userSession){
		...
		XMLResponse xmlResp = dataBase.deleteDocuments(keys, currentUser.getAllUserGroups(), currentUser.getUserID(), completeRemove);
	...
	private ProviderResult undelete(HttpServletRequest request, UserSession userSession){
		...
		IDatabase dataBase = env.getDataBase();	
		User currentUser = userSession.currentUser;
		XMLResponse xmlResp = dataBase.unDeleteDocuments(keys, currentUser.getAllUserGroups(), currentUser.getUserID());
		result.output.append(xmlResp.toXML());
		return result;	
	}
		...
		if (handlerRule.trigger == TriggerType.PROVIDER){
			...
			if (handlerRule.scriptIsValid){
				...
				if (handlerRule.waitResponse){				
					...
					/*AppEnv.logger.verboseLogEntry("Attempt to run handler (ruleid=" + handlerRule.id + ")");
						String result = wfsp.process(handlerRule.getScript());
						if (!handlerRule.showFile){
							output.append(result);
						}else{
							publishAs = PublishAsType.OUTPUTSTREAM;
							filePath = result;
							originalAttachName = new File(result).getName();
						}	*/
	...
	private ProviderResult service(HttpServletRequest request, HttpServletResponse response, UserSession userSession, String key, IRule rule) throws InterruptedException, RuleException, QueryFormulaParserException, DocumentException, DocumentAccessException, ComplexObjectException{
		String element = request.getParameter("element");
		ProviderResult result = new ProviderResult();
		IDatabase db = env.getDataBase();
		...
		if(operation.equalsIgnoreCase("mark_as_read")){
			...
			ua.postMarkRead(docID, docType, userSession.currentUser.getUserID());
		}else if(operation.equalsIgnoreCase("mark_as_unread")){
			ua.postMarkUnread(docID, docType, userSession.currentUser.getUserID());
			...
			if(docType==Const.DOCTYPE_TASK){				
				...
				//int day_diff = RuntimeObjUtil.getDiffBetweenDays(currentDate, ctrlDate, holidays, false);
