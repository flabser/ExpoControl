	...
	public void run() {
		...
		try {			
			...
			String sql = "insert into USERS_ACTIVITY(TYPE, DBID, USERID, EVENTTIME, DOCID, DOCTYPE, VIEWTEXT, DDBID) values ("
				+ UsersActivityType.MODIFIED.getCode() + ",'" + dbID + "', '" + userID + "','" + Database.sqlDateTimeFormat.format(new java.util.Date()) +
				"', " + modifiedDoc.getDocID() + ", " + modifiedDoc.docType + ", '" + RuntimeObjUtil.cutHTMLText(modifiedDoc.getViewText().replace("'", "''"),512) + "', '" + modifiedDoc.getDdbID() + "')";
