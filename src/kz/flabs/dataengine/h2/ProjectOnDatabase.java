...
public class ProjectOnDatabase  extends DatabaseCore implements IProjects, Const{
	...
	public int insertProject(Project doc, Set<String> complexUserID, String absoluteUserID)
		...
		try {
			...
			if (!doc.hasField("recID")){
				...
				ua.postMarkRead(keyProject, doc.docType, absoluteUserID);
			}
	...
	public int updateProject(Project doc, Set<String> complexUserID, String absoluteUserID)throws DocumentAccessException, DocumentException {
		if (doc.hasEditor(complexUserID)) {	
			Project oldDoc = this.getProjectByID(doc.getDocID(), complexUserID, absoluteUserID);
			Connection conn = dbPool.getConnection();
			try {
...
/*		try{
			Statement statBlock = conn.createStatement();
			...
			statBlock.executeUpdate(sqlBlock, Statement.RETURN_GENERATED_KEYS);
	...
	protected int insertCoordinator(int blockKey, Coordinator coordinator, Connection conn){
		try{
			Statement statCoord = conn.createStatement();
			String sqlCoord = "insert into COORDINATORS(BLOCKID, COORDTYPE, COORDINATOR, COORDNUMBER, ISCURRENT, COMMENT, DECISION, COORDATE, " +
					"DECISIONDATE) values (" + blockKey + "," + coordinator.getCoordType() + ", '" + coordinator.getUser() + "', "
					+ coordinator.num + "," + coordinator.isCurrent() + ", '" + coordinator.comment	+ "', "	+ coordinator.decision
					+ ", "	+ coordinator.getCoorDateAsDbFormat() + ", " + coordinator.getDecisionDateAsDbFormat() + ")";

			int res = statCoord.executeUpdate(sqlCoord);
			statCoord.close();
			return res;
		} catch (SQLException e) {
			DatabaseUtil.errorPrint(db.getDbID(), e);
