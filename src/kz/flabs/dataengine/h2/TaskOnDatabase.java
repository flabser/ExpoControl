...
*/
	...
	public int insertTask(Task doc, Set<String> complexUserID, String absoluteUserID) {
		int key = 0;
		int id = doc.getDocID();
	...
	public int updateTask(Task doc, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException {
		if (doc.hasEditor(complexUserID)) {				
			Task oldDoc = this.getTaskByID(doc.getDocID(), complexUserID, absoluteUserID);
			Control ctrl = doc.getControl();
			Connection conn = dbPool.getConnection();
			try {
