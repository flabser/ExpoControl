...
public class Activity implements IActivity {
    ...
    @Override
    public int postStartOfActivity(String viewText, String userID, String nameOfService, String nameOfMethod, String parameters, String springServer, String transaction, Date request_time, Integer minutes) {
        int key = 0;
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            String sql = "INSERT INTO ACTIVITY(viewtext, userid, service_name, method_name, parameters, spring_server, event_time, transaction, request_time, processed_rec) values (" +
                    " ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement pst = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);
            pst.setString(1, viewText);
            pst.setString(2, userID);
    ...
    @Override
    public int postEndOfActivity(Integer activityID) {
        int key = 0;
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            String sql = "UPDATE ACTIVITY set activity_type = ? where id = ?";
            PreparedStatement pst = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);
    ...
    @Override
    public ViewEntryCollection getActivities(String userid, String objectid, String transaction, String service, int pageSize) {
        HashSet<IViewEntry> activityEntries = new HashSet<>();
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            String sql = "select * from activity where userid = ? and parameters = ? and transaction = ? and service_name = ?";
