...
public class Database extends DatabaseCore implements IDatabase, Const {
    ...
    private static final String maindocFields = "MAINDOCS.DOCID, DDBID, AUTHOR, PARENTDOCID, PARENTDOCTYPE, REGDATE, DOCTYPE, LASTUPDATE, VIEWTEXT, " + DatabaseUtil.getViewTextList("") + ", VIEWNUMBER, VIEWDATE, VIEWICON, FORM, HAS_ATTACHMENT ";
    ...
    public void resetTopic(int parentDocID, int parentDocType, Set<String> complexUserID, String absoluteUserID) throws DatabasePoolException, InstantiationException, IllegalAccessException, ClassNotFoundException, ComplexObjectException {
        Connection conn = dbPool.getConnection();
        String tableName = DatabaseUtil.getMainTableName(parentDocType);
        ...
        try {
            ...
            if (rs.next()) {
                this.deleteDocument(Const.DOCTYPE_TOPIC, rs.getInt("TOPICID"), complexUserID, absoluteUserID, true);
    ...
    public Document getMainDocumentByID(int docID, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException, DocumentException, ComplexObjectException {
        ...
        try {
            conn.setAutoCommit(false);
            Statement statement = conn.createStatement();            /*	String sql = "select DISTINCT " + maindocFields + ", " + glossaryFields + " from MAINDOCS, READERS_MAINDOCS, CUSTOM_FIELDS "
                    + "where MAINDOCS.DOCID = READERS_MAINDOCS.DOCID "
					+ "and MAINDOCS.DOCID = CUSTOM_FIELDS.DOCID "
					+ "and MAINDOCS.DOCID = "
					+ docID
					+ " "
                ...
                while (isNextMain || rs.next()) {
                    switch (rs.getInt("TYPE")) {
                        ...
                        case COMPLEX_OBJECT:
                            ...
                            String className = value.substring(0, delimiterPosition);
                            String initString = value.substring(delimiterPosition + 1);
                            Object object = null;
                            try {
                                ...
                                //	throw new ComplexObjectException(ComplexObjectExceptionType.CANNOT_CAST_TO_CLASS, e.getMessage() + ", field=" + fieldName);
    ...
    public String getMainDocumentFieldValueByID(int docID, Set<String> complexUserID, String absoluteUserID, String fieldName) throws DocumentAccessException {
        ...
        try {
            ...
            if (bSysField) {
                sql = "select m." + fieldName + " from MAINDOCS as m where exists "
                        + "(select docid,username from readers_maindocs as rm where"
                        + " rm.docid = " + docID
    ...
    public String getGlossaryCustomFieldValueByID(int docID, String fieldName) {
        ...
        try {
            ...
            String sql = "select cfg.docid, cfg.type, cfg.name, cfg.value, cfg.valueasnumber, cfg.valueasdate, cfg.valueasglossary "
                    ...
                    + "where exists (select docid from GLOSSARY as g where g.docid = " + docID + " and cfg.docid = " + docID + " and cfg.name='" + fieldName + "');";
    ...
    @Override
    public int insertMainDocument(Document doc, Set<String> complexUserID, String absoluteUserID) throws DocumentException {
        int id = doc.getDocID();
        String fieldsAsText = "AUTHOR, REGDATE, DOCTYPE, LASTUPDATE, DDBID, VIEWTEXT, PARENTDOCID, PARENTDOCTYPE, " +
        ...
        try {
            ...
            try {
                ...
                if (!doc.hasField("recID")) {
                    ...
                    ua.postCompose(doc, absoluteUserID);
                    ua.postMarkRead(key, doc.docType, absoluteUserID);
                }
                return key;
            } catch (SQLException e) {
                conn.rollback();
    ...
    public int updateMainDocument(Document doc, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException, DocumentException, ComplexObjectException {
        if (doc.hasEditor(complexUserID)) {
            Document oldDoc = this.getMainDocumentByID(doc.getDocID(), complexUserID, absoluteUserID);
            Connection conn = dbPool.getConnection();
            try {
                ...
                String mainDocUpd = "update MAINDOCS set LASTUPDATE = '"
                        + sqlDateTimeFormat.format(doc.getLastUpdate())
                        + "', VIEWTEXT='" + doc.getViewText().replace("'", "''").trim()
                        + "', DDBID='" + doc.getDdbID()
                    ...
                    switch (field.getTypeAsDatabaseType()) {
                        case TEXT:
                            upCustomFields = "merge into CUSTOM_FIELDS (DOCID, NAME, VALUE, TYPE) key(DOCID, NAME) values(" +
                                    doc.getDocID() + ", '" + field.name + "', '" + field.valueAsText.replace("'", "''").trim() + "', " +
                                    field.getTypeAsDatabaseType() + ")";
                            break;
                        case COMPLEX_OBJECT:
                            upCustomFields = "merge into CUSTOM_FIELDS (DOCID, NAME, VALUE, TYPE) key(DOCID, NAME) values(" +
                                    doc.getDocID() + ", '" + field.name + "', '" + field.valueAsObject.getPersistentValue() + "', " +
                                    field.getTypeAsDatabaseType() + ")";
                            break;
                        case TEXTLIST:
                        ...
                        case DATETIMES:
                            ...
                            try {
                                upCustomFields = "merge into CUSTOM_FIELDS (DOCID, NAME, VALUEASDATE, TYPE) KEY(DOCID, NAME)" +
                                        " VALUES (" + doc.getDocID() + ", '" + field.name + "', '" + Util.convertDateTimeToDerbyFormat(field.valueAsDate) +
                                        "', " + field.getTypeAsDatabaseType() + ")";
                            } catch (DataConversionException e) {
                                Database.logger.errorLogEntry(e + ", field=" + field.name);
                                return -1;
                ...
                IUsersActivity ua = getUserActivity();
                ua.postModify(oldDoc, doc, absoluteUserID);
            ...
            throw new DocumentAccessException(ExceptionType.DOCUMENT_WRITE_ACCESS_RESTRICTED, absoluteUserID);
        }
    }
    ...
    public void updateAccessTables(Connection conn, Document doc, String tableSuffix) throws SQLException {
        ...
        for (String table : accessTables) {
            ...
            if (table.equals(accessTables[0])) {
                PreparedStatement insertion = conn.prepareStatement("INSERT INTO " + table
                        + " (USERNAME, DOCID) VALUES (?, " + doc.getDocID() + ")");
                ...
                PreparedStatement insertion = conn.prepareStatement("INSERT INTO " + table
                        + " (USERNAME, DOCID, FAVORITES) VALUES (?, ?, ?)");
    ...
    @Deprecated
    public ArrayList<BaseDocument> getAllDocuments(int docType, Set<String> complexUserID, String absoluteUserID, int start, int end) throws DocumentException, DocumentAccessException, ComplexObjectException {
        ArrayList<BaseDocument> documents = new ArrayList<BaseDocument>();
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
    ...
    @Deprecated
    public ArrayList<Integer> getAllDocumentsIDS(int docType, Set<String> complexUserID, String absoluteUserID, String[] fields, int start, int end) throws DocumentException, DocumentAccessException {
    ...
    public StringBuffer getDocsByCondition(String sql, Set<String> complexUserID, String absoluteUserID, String fieldsCond, Set<DocID> toExpandResponses, Set<String> toExpandCategory, TagPublicationFormatType publishAs, int page) throws DocumentException {
        StringBuffer xmlContent = new StringBuffer(10000);
    ...
    public StringBuffer getGetOneCategory(Set<String> complexUserID, String absoluteUserID, String addCondition, String fieldsCond, Set<DocID> toExpandResponses, int page) {
        StringBuffer xmlContent = new StringBuffer(10000);
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            ...
            String sql = "SELECT DISTINCT " + maindocFields + " FROM MAINDOCS, READERS_MAINDOCS rm WHERE rm.docid = MAINDOCS.DOCID and " +
                    "rm.USERNAME IN (" + DatabaseUtil.prepareListToQuery(complexUserID) + ")" +
                    addCondition + " ORDER BY MAINDOCS.regdate";
    ...
    public String getDocumentEntry(Connection conn, Set<String> complexUserID, String absoluteUserID, ResultSet rs, String fieldsCond, Set<DocID> toExpandResponses, int page) throws SQLException, DocumentException {
        ...
        StringBuffer value = new StringBuffer("<entry isread=\"" + usersActivity.isRead(conn, docID, docType, absoluteUserID) + "\" " +
                "hasattach=\"" + Integer.toString(rs.getInt("HAS_ATTACHMENT")) + "\"  id=\"" + id + "\" doctype=\"" + docType + "\"  " +
                "docid=\"" + docID + "\" favourites=\"" + isFavourites(conn, docID, docType, emp) + "\" " +
                "url=\"Provider?type=edit&amp;element=" + resolveElement(docType) + "&amp;id=" + rs.getString("FORM") + "&amp;key=" + docID + "&amp;docid=" + rs.getString("DDBID") + "&amp;page=" + page + "\"" +
     ...
     */
    @Override
    public ArrayList<BaseDocument> getDocumentsByCondition(String form, String query, Set<String> complexUserID, String absoluteUserID) throws DocumentException, DocumentAccessException, QueryFormulaParserException, ComplexObjectException {
    ...
    @Override
    public int getDocumentsCountByCondition(String query, Set<String> complexUserID, String absoluteUserID) throws DocumentException, DocumentAccessException, QueryFormulaParserException {
        FormulaBlocks preparedQueryFormula = new FormulaBlocks(query, QueryType.DOCUMENT);
        QueryFormula queryFormula = new QueryFormula("", preparedQueryFormula);
        return getDocsCountByCondition(queryFormula, complexUserID, absoluteUserID);
    }

    public ArrayList<BaseDocument> getDocumentsByCondition(IQueryFormula blocks, Set<String> complexUserID, String absoluteUserID, int limit, int offset, Set<DocID> expandedThread) throws DocumentException, DocumentAccessException, ComplexObjectException {
    ...
    @Override
    public ArrayList<BaseDocument> getDocumentsByCondition(IQueryFormula blocks, Set<String> complexUserID, String absoluteUserID, int limit, int offset) throws DocumentException, DocumentAccessException, ComplexObjectException {
        ArrayList<BaseDocument> docs = new ArrayList<BaseDocument>();
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
    ...
    public boolean hasResponse(Connection conn, int docID, int docType, Set<String> complexUserID, String absoluteUserID) {
        try {
            ...
            String sql = "select m.docid, m.doctype, m.form from maindocs as m " +
                    " inner join readers_maindocs as rm " +
                    "     on m.docid = rm.docid " +
                    " where m.parentdocid = " + docID + " and m.parentdoctype = " + docType + " and rm.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ")" +
                    " and m.form != 'discussion' " +
                    " union all " +
                    " select t.docid, t.doctype, t.form from tasks as t " +
                    " inner join readers_tasks as rt " +
                    "     on t.docid = rt.docid " +
                    " where t.parentdocid = " + docID + " and t.parentdoctype = " + docType + " and rt.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ")" +
                    " union all " +
                    " select e.docid, e.doctype, e.form from executions as e " +
                    " inner join readers_executions as re " +
                    "     on e.docid = re.docid " +
                    " where e.parentdocid = " + docID + " and e.parentdoctype = " + docType + " and re.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ")";
    ...
    public boolean hasResponse(int docID, int docType, Set<String> complexUserID, String absoluteUserID) {
        ...
        try {
            ...
            String sql = "select m.docid, m.doctype, m.form from maindocs as m, readers_maindocs as rm " +
                    ...
                    " and m.form != 'discussion' " +
                    " union all " +
                    " select t.docid, t.doctype, t.form from tasks as t, readers_tasks as rt" +
                    " where t.parentdocid = " + docID + " and t.parentdoctype = " + docType + " and rt.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ")" +
                    " union all " +
    ...
    public void deleteDocument(String docID, Set<String> complexUserID, String absoluteUserID, boolean completely) throws DocumentException, DocumentAccessException, SQLException, DatabasePoolException, InstantiationException, IllegalAccessException, ClassNotFoundException, ComplexObjectException {
        Connection conn = dbPool.getConnection();
        BaseDocument doc = getDocumentByDdbID(docID, complexUserID, absoluteUserID);
        if (doc == null) {
            throw new DocumentAccessException(ExceptionType.DOCUMENT_READ_RESTRICTED, absoluteUserID);
        }
        try {
            ...
            if (docType != DOCTYPE_GLOSSARY && docType != DOCTYPE_GROUP) {
                if (!doc.hasEditor(complexUserID)) {
                    throw new DocumentAccessException(ExceptionType.DOCUMENT_DELETE_RESTRICTED, absoluteUserID);
                }
            } else {
                if (this.getGlossaries().hasResponse(conn, doc.getDocID(), doc.docType, complexUserID, absoluteUserID) || this.getGlossaries().inUse(conn, doc.getDocID(), doc.docType)) {
                    throw new DocumentAccessException(ExceptionType.DELETING_RESTRICTED_CAUSED_RELATED_DOCUMENTS, absoluteUserID);
                }
            }
            ...
            if (docType == DOCTYPE_TOPIC) {
                sql = "DELETE FROM FORUM_TREE_PATH WHERE DESCENDANT IN (SELECT DESCENDANT FROM FORUM_TREE_PATH WHERE ANCESTOR IN (SELECT DOCID FROM POSTS WHERE PARENTDOCID = " + doc.getDocID() + " AND PARENTDOCTYPE = " + Const.DOCTYPE_TOPIC + "))";
                statement.addBatch(sql);
                sql = "delete from posts where docid in (select docid from posts except (select ancestor from forum_tree_path union select descendant from forum_tree_path))";
                statement.addBatch(sql);
            ...
            if (!completely) {
                for (BaseDocument resp : responses) {
                    ...
                    sql = "DELETE FROM " + DatabaseUtil.getMainTableName(resp.docType) + " WHERE " + DatabaseUtil.getPrimaryKeyColumnName(resp.docType) + " = " + Integer.toString(resp.getDocID());
                    statement.addBatch(sql);
                }
            ...
            sql = "DELETE FROM " + DatabaseUtil.getMainTableName(doc.docType) + " WHERE " + DatabaseUtil.getPrimaryKeyColumnName(doc.docType) + " = " + Integer.toString(doc.getDocID());
            statement.addBatch(sql);
        ...
        IUsersActivity ua = getUserActivity();
        if (!completely) {
            ua.postDelete(doc, absoluteUserID);
        } else {
            ua.postCompletelyDelete(doc, absoluteUserID);
        }
    }

    public void deleteDocument(int docType, int docID, Set<String> complexUserID, String absoluteUserID, boolean completely) throws DocumentException, DocumentAccessException, SQLException, DatabasePoolException, InstantiationException, IllegalAccessException, ClassNotFoundException, ComplexObjectException {
        @Deprecated
        ...
        try {
            ...
            if (checkAuthors && (!doc.hasEditor(complexUserID))) {
                throw new DocumentAccessException(ExceptionType.DOCUMENT_DELETE_RESTRICTED, absoluteUserID);
            }
            if (docType == DOCTYPE_GLOSSARY) {
                if (this.getGlossaries().hasResponse(conn, docID, docID, complexUserID, absoluteUserID)) {
                    throw new DocumentAccessException(ExceptionType.DELETING_RESTRICTED_CAUSED_RELATED_DOCUMENTS, absoluteUserID);
                }
            }
            if (docType == DOCTYPE_POST) {
            ...
            if (docType == DOCTYPE_TOPIC) {
                sql = "DELETE FROM FORUM_TREE_PATH WHERE DESCENDANT IN (SELECT DESCENDANT FROM FORUM_TREE_PATH WHERE ANCESTOR IN (SELECT DOCID FROM POSTS WHERE PARENTDOCID = " + docID + " AND PARENTDOCTYPE = " + Const.DOCTYPE_TOPIC + "))";
                statement.addBatch(sql);
                sql = "delete from posts where docid in (select docid from posts except (select ancestor from forum_tree_path union select descendant from forum_tree_path))";
                statement.addBatch(sql);
    ...
    public void recoverDocument(BaseDocument recoverDoc, int recEntryID) throws DocumentAccessException, DocumentException, LicenseException, ComplexObjectException {
    ...
    @Override
    public boolean unDeleteDocument(String id, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException, ComplexObjectException {
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            Statement s = conn.createStatement();
            ...
            if (rs.next()) {
                ...
                IUsersActivity ua = getUserActivity();
                ua.postUndelete(doc, absoluteUserID);
            }
            rs.close();
            s.close();
    ...
    @Override
    public boolean unDeleteDocument(int id, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException, ComplexObjectException {
        Connection conn = dbPool.getConnection();
        try {
            conn.setAutoCommit(false);
            Statement s = conn.createStatement();
    ...
    public StringBuffer getPatches() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String name = rs.getString("NAME");
                xmlContent.append("<entry doctype=\"PATCH\" " + XMLUtil.getAsAttribute("viewtext", "time=" + processedTime + ", hash=" + hashCode + ", descritpion=" + description + ", name=" + name) + ">" +
                        "<viewtext>" + XMLUtil.getAsTagValue("time=" + processedTime + ", hash=" + hashCode + ", description=" + description + ", name=" + name) + "</viewtext>" +
                        "<processedtime>" + processedTime + "</processedtime><hash>" + hashCode + "</hash><name>" + name + "</name><description>" + XMLUtil.getAsTagValue(description) + "</description></entry>");
            }
            rs.close();
            statement.close();
            conn.commit();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                xmlContent.append("<entry doctype=\"COUNTER\" " + XMLUtil.getAsAttribute("viewtext", "key=" + keys + ", last number=" + lastNum) + ">" +
                        "<viewtext>" + XMLUtil.getAsTagValue("key=" + keys + ", last number=" + lastNum) + "</viewtext>" +
                        "<keys>" + keys + "</keys><lastnum>" + lastNum + "</lastnum></entry>");
    ...
    public boolean hasDocumentByComplexID(int docID, int docType) {
        ...
        try {
            ...
            switch (docType) {
                case DOCTYPE_MAIN:
                    String sql = "select * from MAINDOCS where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    PreparedStatement pst = conn.prepareStatement(sql);
                    ResultSet rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_TASK:
                    sql = "select * from TASKS where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_EXECUTION:
                    sql = "select * from EXECUTIONS where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_PROJECT:
                    sql = "select * from PROJECTS where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_GLOSSARY:
                    sql = "select * from GLOSSARY where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                ...
                case DOCTYPE_ORGANIZATION:
                    sql = "select * from ORGANIZATIONS where ORGID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                ...
                case DOCTYPE_DEPARTMENT:
                    sql = "select * from DEPARTMENTS where DEPID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_EMPLOYER:
                    sql = "select * from EMPLOYERS where EMPID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_TOPIC:
                    sql = "select * from TOPICS where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
                    ...
                    break;
                case DOCTYPE_POST:
                    sql = "select * from POSTS where DOCID = " + docID
                            + " and DOCTYPE = " + docType;
                    pst = conn.prepareStatement(sql);
                    rs = pst.executeQuery();
                    while (rs.next()) {
    ...
    @Override
    public BaseDocument getDocumentByComplexID(int docType, int docID, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException, DocumentException, ComplexObjectException {
        switch (docType) {
            case DOCTYPE_MAIN:
    ...
    @Override
    public BaseDocument getDocumentByDdbID(String ddbID, Set<String> complexUserID, String absoluteUserID) throws DocumentException, DocumentAccessException, ComplexObjectException {
        Statement statement = null;
        BaseDocument doc = null;
        Connection conn = dbPool.getConnection();
        String sql = "select m.docid, m.doctype from maindocs as m, readers_maindocs as rm where m.docid = rm.docid and rm.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and ddbid='" + ddbID + "' " +
                "union " +
                "select t.docid, t.doctype from tasks as t, readers_tasks as rt where t.docid = rt.docid and rt.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and ddbid='" + ddbID + "' " +
                "union " +
                "select e.docid, e.doctype from executions as e, readers_executions as re where e.docid = re.docid and re.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and ddbid='" + ddbID + "' " +
                "union " +
                "select p.docid, p.doctype from projects as p, readers_projects as rp where p.docid = rp.docid and rp.username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and ddbid='" + ddbID + "' " +
                "union " +
    ...
    @Override
    public IViewEntry getDocumentByDocID(String ddbID, Set<String> complexUserID, String absoluteUserID) throws DocumentException, DocumentAccessException {
        Statement statement = null;
        ...
        if (docID != 0 && docType != DOCTYPE_UNKNOWN) {
            ...
            try {
                ...
                String sql = "SELECT distinct resp.docid, doctype, form, resp.viewtext, regdate, has_attachment, resp.allcontrol, ctype"
                        ...
                        + " FROM EXECUTIONS resp, READERS_EXECUTIONS r WHERE parentdocid = "
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    xmlContent.append("<entry isread=\"" + usersActivity.isRead(conn, respDocID, repsDocType, absoluteUserID) + "\" hasattach=\"" + Integer.toString(rs.getInt("HAS_ATTACHMENT")) + "\" doctype=\"" + repsDocType
                            ...
                            + respDocID + "\"" + (!form.equalsIgnoreCase("KI") ? " allcontrol =\"" + allControl + "\"" : "")
        ...
        if (docID != 0 && docType != DOCTYPE_UNKNOWN) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    String form = rs.getString("FORM");
                    xmlContent.append("<entry isread=\"" + usersActivity.isRead(conn, respDocID, repsDocType, absoluteUserID) + "\" hasattach=\"" + Integer.toString(rs.getInt("HAS_ATTACHMENT")) + "\" doctype=\"" + repsDocType
                            + "\"  docid=\"" + respDocID + "\" form=\"" + form + "\" url=\"Provider?type=edit&amp;element=document&amp;id=" + form + "&amp;key="
    ...
    public _ViewEntryCollection getCollectionByCondition(ISelectFormula condition, User user, int pageNum, int pageSize, Set<DocID> toExpandResponses, RunTimeParameters parameters, boolean checkResponse, String responseQueryCondition) {
        ...
        try {
            ...
            String sql = condition.getCondition(users, pageSize, offset, parameters.getFilters(), parameters.getSorting(), checkResponse, responseQueryCondition);
            ResultSet rs = s.executeQuery(sql);
            if (rs.next()) {
    ...
    @Override
    public _ViewEntryCollection getCollectionByCondition(ISelectFormula condition, User user, int pageNum, int pageSize, Set<DocID> toExpandResponses, RunTimeParameters parameters, boolean checkResponse) {
        ViewEntryCollection coll = new ViewEntryCollection(pageSize, user, parameters);
        Set<String> users = user.getAllUserGroups();
        Connection conn = dbPool.getConnection();
        try {
    ...
    public int parseFile(File parentDir, File dir, HashMap<Integer, Integer> linkOldNew) {
        ...
        try {
            ...
            if (!XMLUtil.getTextContent(xmlDoc, "document/@parentdocid").trim().equals("0")) {
                int parentDocId = parseFile(parentDir, new File(parentDir.getAbsolutePath() + File.separator + "doc_" + XMLUtil.getTextContent(xmlDoc, "document/@parentdocid").trim()), linkOldNew);
                if (parentDocId < 0) {
    ...
    public void fillAccessRelatedField(Connection conn, String accessTableSuffix, int docID, Document doc) throws SQLException {
        ...
        String getAuthSQL = "select * from AUTHORS_" + accessTableSuffix
                + " where DOCID = " + docID;
    ...
    @Override
    public ArrayList<BaseDocument> getResponses(int docID, int docType, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException, DocumentException, ComplexObjectException {
        ArrayList<BaseDocument> documents = new ArrayList<BaseDocument>();
        if (docID != 0 && docType != DOCTYPE_UNKNOWN) {
            Connection conn = dbPool.getConnection();
            try {
                ...
                String sql = "select distinct resp.docid, doctype, form, viewtext, regdate, username, has_attachment FROM TASKS resp, READERS_TASKS r where PARENTDOCID="
                        + docID + " and PARENTDOCTYPE=" + docType + " and resp.DOCID = r.DOCID and r.USERNAME IN (" + ids + ")"
                        + " UNION SELECT distinct resp.docid, doctype, form, viewtext, regdate, username, has_attachment FROM EXECUTIONS resp, READERS_EXECUTIONS r WHERE PARENTDOCID="
                        + docID + " and PARENTDOCTYPE=" + docType + " and resp.DOCID = r.DOCID and r.USERNAME IN (" + ids + ")"
                        + " UNION SELECT distinct resp.docid, doctype, form, viewtext, regdate, username, has_attachment FROM MAINDOCS resp, READERS_MAINDOCS r WHERE PARENTDOCID="
                        + docID + " and PARENTDOCTYPE=" + docType + " and resp.DOCID = r.DOCID and r.USERNAME IN (" + ids + ")"
                        + " UNION SELECT distinct resp.docid, doctype, form, viewtext, regdate, '" + absoluteUserID + "' as username, 0 as has_attachment from GLOSSARY resp where parentdocid="
    ...
    public XMLResponse deleteDocuments(List<DocID> docs, Set<String> complexUserID, String absoluteUserID, boolean completely) {
        ...
        for (DocID id : docs) {
            try {
                ...
                this.deleteDocument(id.type, id.id, complexUserID, absoluteUserID, completely);
                countDeleted++;
    ...
    public XMLResponse unDeleteDocuments(List<DocID> docs, Set<String> complexUserID, String absoluteUserID) {
        XMLResponse result = new XMLResponse(ResponseType.UNDELETE_DOCUMENT);
        StringBuffer messages = new StringBuffer(1000);
        int countRestored = 0, countNotRestored = 0;
        for (DocID id : docs) {
            try {
                boolean isRestore = this.unDeleteDocument(id.id, complexUserID, absoluteUserID);
                if (isRestore) {
                    countRestored++;
                }
            } catch (Exception e) {
    ...
    public ArrayList<ViewEntry> getGroupedEntries(String fieldName, int offset, int pageSize, User user) {
        ...
        try {
            ...
            if (!fieldName.contains("view")) {
                sql = "select cf." + result[1] + ", count(cf." + result[1] + ") from custom_fields as cf where cf.name = '" + result[0] + "' and " + " cf." + result[1] + " is not null and " +
                        " cf.docid in (select docid from readers_maindocs as rm where rm.username in (" + DatabaseUtil.prepareListToQuery(users) + ")) " +
                        " group by cf." + result[1] + " order by cf." + result[1] + " limit " + pageSize + " offset " + offset;
            } else {
        ...
        try {
            ...
            if (isSystemField(fieldName)) {
                sql = "select distinct MAINDOCS.DOCTYPE, MAINDOCS.DOCID from MAINDOCS where MAINDOCS.DOCID in " +
                        ...
                        "(form= '" + form + "' and MONTH(" + fieldName + ") = " + month + " ) limit " + pageSize + " offset " + offset;
            } else {
                sql = "select distinct MAINDOCS.DOCTYPE, MAINDOCS.DOCID from MAINDOCS where MAINDOCS.DOCID in " +
                        ...
                        "(select docid from CUSTOM_FIELDS where MAINDOCS.DOCID = CUSTOM_FIELDS.DOCID AND CUSTOM_FIELDS.NAME='" + fieldName + "' and MONTH(CUSTOM_FIELDS.VALUEASDATE) = " + month + ")) and " +
    ...
    public StringBuffer getFavorites(IQueryFormula nf, Set<String> complexUserID, String absoluteUserID, int offset, int pageSize, String fieldsCond, Set<DocID> toExpandResponses, Set<String> toExpandCategory, TagPublicationFormatType publishAs, int page) {
        ...
        try {
            Statement s = conn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            String sql = "select has_attachment, form, ddbid, docid, doctype, viewtext, " + DatabaseUtil.getViewTextList("") + ", viewnumber, viewdate from maindocs where docid in (select docid from readers_maindocs where username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and favorites = 1) " +
                    " union " +
                    " select has_attachment, form, ddbid, docid, doctype, viewtext, " + DatabaseUtil.getViewTextList("") + ", viewnumber, viewdate  from tasks where docid in (select docid from readers_tasks where username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and favorites = 1) " +
                    " union " +
                    " select has_attachment, form, ddbid, docid, doctype, viewtext, " + DatabaseUtil.getViewTextList("") + ", viewnumber, viewdate  from executions where docid in (select docid from readers_executions where username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and favorites = 1) " +
                    " union " +
                    " select has_attachment, form, ddbid, docid, doctype, viewtext, " + DatabaseUtil.getViewTextList("") + ", viewnumber, viewdate  from projects where docid in (select docid from readers_projects where username in (" + DatabaseUtil.prepareListToQuery(complexUserID) + ") and favorites = 1) " +
    ...
    public int isFavourites(Connection conn, int docID, int docType, Employer user) {
        int isFavour = 0;
        try {
            String sql = "select favorites from READERS_MAINDOCS where username in (" + DatabaseUtil.prepareListToQuery(user.getAllUserGroups()) + ") and docid = " + docID;
    ...
    public int isFavourites(int docID, int docType, String userName) {
        ...
        Connection conn = dbPool.getConnection();
        try {
            //String sql = "select docid from users_activity where (type = " + UsersActivityType.MARKED_AS_FAVORITE.getCode() + " or type = " + UsersActivityType.UNMARKED_AS_FAVORITE.getCode() + ") and dbid = '" + this.getDbID() + "' and docid = " + docID + " and doctype = " + docType + " and userid = '" + userName + "' group by docid having count(docid) % 2 <> 0";
            Employer user = this.getStructure().getAppUser(userName);
            String sql = "select favorites from " + DatabaseUtil.getReadersTableName(docType) + " where username  in (" + DatabaseUtil.prepareListToQuery(user.getAllUserGroups()) + ") and docid = " + docID;
    ...
    public boolean clearDocuments() {
        ...
        try {
            ...
            String sql = "select CONSTRAINT_NAME, SQL from information_schema.CONSTRAINTS where LCASE(table_name) = '" + tableType + "' and LCASE(column_list) = 'docid'";
            Statement s = conn.createStatement();
            ResultSet rs = s.executeQuery(sql);
            ...
            for (String tableName : tables) {
                sql = "select CONSTRAINT_NAME, SQL from information_schema.CONSTRAINTS where LCASE(table_name) = '" + tableType + tableName + "' and LCASE(column_list) = 'docid'";
                rs = s.executeQuery(sql);
            ...
            tableType = "readers_";
            for (String tableName : tables) {
                sql = "select CONSTRAINT_NAME, SQL from information_schema.CONSTRAINTS where LCASE(table_name) = '" + tableType + tableName + "' and LCASE(column_list) = 'username,docid'";
                rs = s.executeQuery(sql);
            ...
            //tasks
            tableType = "";
            sql = "select CONSTRAINT_NAME, SQL from information_schema.CONSTRAINTS where LCASE(table_name) = '" + tableType + tables[4] + "' and LCASE(column_list) = 'username,docid'";
            rs = s.executeQuery(sql);
            ...
            s.execute(sql);
            //sql = "DELETE FROM PROJECTS WHERE DOCID IN (SELECT DOCID FROM MAINDOCS)";
            sql = "DELETE FROM PROJECTS";
            s.execute(sql);
