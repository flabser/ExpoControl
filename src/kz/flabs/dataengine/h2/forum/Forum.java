...
public class Forum extends DatabaseCore implements IForum, Const {
   ...
   /* public Topic getTopicByPostID(BaseDocument post, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException{
	...
	@Override
	public int insertTopic(Topic topic, Set<String> complexUserID, String absoluteUserID) {
		int key = 0;
		Connection conn = dbPool.getConnection();		
		try {
			conn.setAutoCommit(false);
			...
			ua.postCompose(topic, absoluteUserID);
			ua.postMarkRead(key, topic.docType, absoluteUserID);
		} catch (SQLException e) {
			DatabaseUtil.debugErrorPrint(e);
			return -1;
		} catch (Exception e) {
	...
	@Override
	public int updateTopic(Topic doc, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException {
		if (doc.hasEditor(complexUserID)) {				
			Topic oldDoc = this.getTopicByID(doc.getDocID(), complexUserID, absoluteUserID);
	...
	@Override
	public int insertPost(Post post, Set<String> complexUserID, String absoluteUserID) {
		int key = 0;
		Connection conn = dbPool.getConnection();		
		try {
			conn.setAutoCommit(false);
			...
			s.close();
			IUsersActivity ua = db.getUserActivity();
			ua.postCompose(post, absoluteUserID);
			ua.postMarkRead(key, post.docType, absoluteUserID);
		} catch (SQLException e) {
			DatabaseUtil.debugErrorPrint(e);
			return -1;
		} catch (Exception e) {
	...
	@Override
	public int updatePost(Post doc, Set<String> complexUserID, String absoluteUserID) throws DocumentAccessException {
		if (doc.hasEditor(complexUserID)) {				
			Topic oldDoc = this.getTopicByID(doc.getDocID(), complexUserID, absoluteUserID);
