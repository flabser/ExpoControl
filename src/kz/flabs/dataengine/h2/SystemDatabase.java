...
public class SystemDatabase implements ISystemDatabase, Const {
	...
	private static String connectionURL = "jdbc:h2:system_data" + File.separator + "system_data;MVCC=TRUE;AUTO_SERVER=" + (Environment.debugMode == RunMode.ON ? "TRUE" : "FALSE");
	...
	public void removeAppEntry(User user) {
		...
		try {
			...
			String sql = "DELETE FROM ENABLEDAPPS WHERE DOCID = (SELECT DOCID FROM USERS WHERE USERID = '" + user.getUserID() + "') AND APP = '" + env.appType + "'";
			s.execute(sql);
			conn.commit();
...
//	private boolean hasColumn(String nameTable, String nameColumn) throws SQLException{
//		Connection conn = dbPool.getConnection();
//		conn.setAutoCommit(false);
//		Statement s = conn.createStatement();
...
//	}
	...
	private String getUsersDDE(){
		String createTable="create table USERS(DOCID int generated by default as identity PRIMARY KEY, " +
				...
				"ISOBSERVER int, " +
				"LOGINHASH int, " +
                "PUBLICKEY varchar(6144)," +
				"PWDHASH VARCHAR(1024))";

		createTable += ";CREATE TABLE TEMP_USERS ( " +
	...
	private String getEnabledAppDDE(){
		String dde = "create table ENABLEDAPPS(ID int generated by default as identity PRIMARY KEY, " +
				"DOCID int, " +
				"APP varchar(32), " +
				"LOGINMODE int, " +
	...
	private String getQADDE(){
		String dde = "create table QA(ID int generated by default as identity PRIMARY KEY, " +
				"DOCID int, " +
				"APP varchar(32), " +
	...
	private String getHolidaysDDE() {
		String dde = "CREATE TABLE HOLIDAYS (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, " +
				" COUNTRY VARCHAR(10) , " +
				" TITLE VARCHAR(64), " +
				" REPEAT INT, " +
				" STARTDATE TIMESTAMP, " +
	...
	public int insert(User user) {
		...
		try{
			...
			if(rs.next())
					key = rs.getInt(1) + 1;	
			String insertUser = "insert into USERS(DOCID, USERID, EMAIL, INSTMSGADDR, ISADMIN, LOGINHASH, PUBLICKEY, PWDHASH )" +
						"values(" + 
					    key + ", " +
					    "'" + user.getUserID() + "', "  + 
						"'" + user.getEmail() + "','" + user.getInstMsgAddress() + "'" +
						"," + user.getIsAdmin() + "," + 
...
//			}
	...
	public ArrayList<Holiday> getHolidays(int start, int end) {
        ...
        try {
            String selectHolidays = "select * from HOLIDAYS " +
            		               "where ID >="+start+ " and ID <="+end;
            PreparedStatement pst = conn.prepareStatement(selectHolidays);
            rs = pst.executeQuery();
            
            while(rs.next()){
                h = new Holiday(rs.getInt("ID"),
                                rs.getString("COUNTRY"), 
                                rs.getString("TITLE"), 
                                rs.getInt("REPEAT"), 
                                rs.getDate("STARTDATE"), 
                                rs.getInt("CONTINUING"), 
                                rs.getDate("ENDDATE"), 
                                rs.getInt("IFFALLSON"), 
	...
	public int update(Holiday h) {
        ...
        try {
            ...
            String insertHoliday = "update HOLIDAYS set "+
                    "COUNTRY = " + (h.getCountry()!=null? "'" + h.getCountry()+"'" :"null")+", "+
                    "TITLE = " + (h.getTitle()!=null? "'" + h.getTitle()+"'" :"null")+", "+
                    "REPEAT = " + h.getRepeat()+", "+
                    "STARTDATE = " + (h.getStartDate()!=null? "'" + h.getStartDate()+"'" :"null")+", "+
                    "CONTINUING = " + h.getContinuing()+", "+
                    "ENDDATE = " + (h.getEndDate()!=null? "'" + h.getEndDate()+"'" :"null")+", "+
                    "IFFALLSON = " + h.getIfFallSon()+", "+
                    "COMMENT = " + (h.getComment()!=null? "'" + h.getComment()+"'" :"null")+" " + 
                    "where ID = " + h.getId();
	...
	public int insert(Holiday h) {
        ...
        try {
            ...
            String insertHoliday = "insert into HOLIDAYS (" +
            		"ID, COUNTRY, TITLE, REPEAT, STARTDATE, CONTINUING, ENDDATE, IFFALLSON, COMMENT) values(" + 
            		h.getId()+", "+
            		(h.getCountry()!=null? "'" + h.getCountry()+"'" :"null")+", "+
            		(h.getTitle()!=null? "'" + h.getTitle()+"'" :"null")+", "+
            		h.getRepeat()+", "+
            		(h.getStartDate()!=null? "'" + h.getStartDate()+"'" :"null")+", "+
            		h.getContinuing()+", "+
            		(h.getEndDate()!=null? "'" + h.getEndDate()+"'" :"null")+", "+
            		h.getIfFallSon()+", "+
