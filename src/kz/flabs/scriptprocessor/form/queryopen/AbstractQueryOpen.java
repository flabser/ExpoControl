...
public abstract class AbstractQueryOpen extends ScriptEvent implements IQueryOpenScript {	
    ...
    public void publishDocumentValue(String entryName, int idValue) throws _Exception{
		try{
			Document doc = env.getDataBase().getMainDocumentByID(idValue, Const.supervisorGroupAsSet, Const.sysUser);
			if(doc != null){
				toPublish.add(new ScriptShowField(entryName, idValue, doc.getViewText().replace("&", "&amp;")));
			}else{
				if(idValue > -1) AppEnv.logger.warningLogEntry("Value \"" + idValue + "\" of the document has not found");
			}
		}catch(IndexOutOfBoundsException e){
			throw new _Exception(_ExceptionType.SCRIPT_ENGINE_ERROR, entryName);
    ...
    public void publishDocumentValue(String entryName, ArrayList<Integer> vals) throws _Exception {
        try{
            for (Integer val : vals) {
                publishDocumentValue(entryName, val);
            }
        }catch(IndexOutOfBoundsException e){
            throw new _Exception(_ExceptionType.SCRIPT_ENGINE_ERROR, entryName);
        }
