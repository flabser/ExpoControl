...
public class UserSession implements Const, ICache {
    ...
    public void setObject(String name, StringBuffer obj) {
        ...
        cache.put(name, obj);
        if (jses != null) jses.setAttribute("cache", cache);
    ...
    public class HistoryEntryCollection {
        ...
        public void add(HistoryEntry entry) throws UserException {
            if (history.size() == 0 || (!history.getLast().equals(entry))) {
                history.add(entry);
                if (entry.isPageURL) pageHistory.add(entry);
            }
        ...
        public boolean remove(Collection<Filter> col) throws DocumentException {
            ...
            if (history.isEmpty()) {
                /*HistoryEntry redirect = new HistoryEntry("outline", homeURL, "Home page");
				history.add(redirect);*/
            }
            return result;
        }
