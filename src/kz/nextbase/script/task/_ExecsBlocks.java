    ...
    /*StringBuffer xmlContent = new StringBuffer(10000);
		...
		if (!execs.getExecutors().isEmpty()) {
			...
			for (int i = 0; i < execsList.size(); i++) {
				...
				xmlContent.append("<entry><num>" + e.num + "</num><userid>" + e.userID + "</userid>");
				IStructure struct = ses.getCurrentDatabase().getBaseObject().getStructure();
				Employer emp = struct.getAppUser(e.userID);
				if(emp != null){
					xmlContent.append("<shortname attrval=\"" + emp.getUserID() + "\">" + emp.getShortName() + "</shortname>");
					xmlContent.append("<fullname attrval=\"" + emp.getUserID() + "\">" + emp.getFullName() + "</fullname>");
				}else{
					xmlContent.append("<shortname attrval=\"" + e.userID + "\"></shortname>");
					xmlContent.append("<fullname attrval=\"" + e.userID + "\"></fullname>");
