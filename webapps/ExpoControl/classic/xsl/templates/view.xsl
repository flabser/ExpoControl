...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="header-page">	
		<div style="position:absolute; top:0px; left:0px; width:100%; background:url(classic/img/header-background.png); height:75px; border-bottom:1px solid #59A3CB; z-index:2">
			<span style="float: left; font-family: Arial Cyr;">
				<img src="/SharedResources/logos/expo2017logo.png" style="margin:0px 10px 0px 15px;"/>
				<font style="font-size:2em; vertical-align:18px; color:#444344; margin-left:15px;">EXPO</font>
				<font style="font-size:1.4em; vertical-align:18px; color:#444344; margin-left:5px;"> Контроль доступа </font>
			</span>
			<span style="float:right; padding:5px 5px 5px 0px">
				<a id="currentuser" target="_parent" href="Provider?type=edit&amp;element=userprofile&amp;id=userprofile" class="no_underline" style="text-decoration:none; color:#444344; font: 11px Tahoma; font-weight:bold">
					<xsl:attribute name="title" select="page/captions/view_userprofile/@caption"/>
					<xsl:value-of select="@username"/>
				</a>
				<a target="_parent" href="Logout" title="{page/captions/logout/@caption}" id="logout" style="text-decoration:none; color:#444344; font:11px Tahoma; font-weight:bold; margin-left:15px">
    ...
    <xsl:template name="outline-menu-page">
        <div id="outline">
            <div id="outline-container" style="width:303px; background: none repeat scroll 0 0 #47a3da;">
            	<div style="height:40px; background: none repeat scroll 0 0 #0d77b6; color: #afdefa; font-size: 1.9em; font-weight: 300; margin: 0; padding: 20px;">
            		Меню
            	</div>
                    ...
                    <div>
                        ...
                        <div class="outlineEntry" id="{@id}">
                            <xsl:for-each select="//response/content/outline/entry">
                                <div class="entry treeentry" style="width:301px; background: none repeat scroll 0 0 #47a3da; border-bottom: 1px solid #258ecd; padding: 1em; cursor:pointer">
                                	<xsl:attribute name="onclick">javascript:window.location.href='<xsl:value-of select="@url"/>'</xsl:attribute>
                                    ...
                                    <a href="{@url}" style="width:301px; height:100%; vertical-align:top; text-decoration:none !important; ; color:#fff !important" title="{@hint}">
                                        <div class="viewlink">
                                            <xsl:if test="/request/@id = @id">
	...
	<xsl:template name="pagenavig">
	 ...
	 <xsl:template name="sortingcellpage">
		...
		<font style="vertical-align:2px; cursor:pointer; color:#fff; font-size:14px; font-family:verdana">
<!-- 			<xsl:attribute name="onclick">javascript:sorting('<xsl:value-of select="/request/@id"/>','<xsl:value-of select="$namefield"/>','asc')</xsl:attribute> -->
			<xsl:if test="$sortorder = 'ASC' and $sortmode = 'ON'">
				<xsl:attribute name="onclick">javascript:sorting('<xsl:value-of select="/request/@id"/>','<xsl:value-of select="$namefield"/>','desc')</xsl:attribute>
			</xsl:if>
