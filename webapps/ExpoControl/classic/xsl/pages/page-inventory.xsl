...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="//content/invent/entry">
		<xsl:variable name="num" select="position()"/>
		<tr title="{@viewtext}" class="{@docid}" id="{@docid}{@doctype}" style="height:40px; color:#1a3dc1">
			...
			<!-- Наименование -->
			<td  style="border:1px solid #ccc;width:400px; padding-left:10px">
				<div style="overflow:hidden; width:99%;">
					<xsl:value-of select="@viewtext"/>
				</div>
			</td>
			<!-- Общее количество до ревизии -->
			<td  style="border:1px solid #ccc;width:160px; padding-left:10px">
				<div style="overflow:hidden; width:99%;">
					<xsl:value-of select="@count"/>
				</div>
			</td>
			<!-- Количество после ревизии -->
			<td  style="border:1px solid #ccc;width:160px; padding-left:10px">
				<div style="overflow:hidden; width:99%;">
					<xsl:value-of select="@byrev"/>
				</div>
			</td>
			<!-- Количество до ревизии -->
			<td  style="border:1px solid #ccc;min-width:280px; word-wrap:break-word; padding-left: 10px">
					    		...
					    		<xsl:value-of select="//current_outline_entry/response/content/entry"/>
									...
									<div id="tableheader">
										<table class="viewtable" id="viewtable" width="100%" style="">
											<tr class="th">
												<td style="width:400px; padding-left:10px" class="thcell">
												...
												<td style="width:160px; padding-left:10px" class="thcell">
													<xsl:call-template name="sortingcellpage">
														<xsl:with-param name="namefield" select="'count'"/>
														<xsl:with-param name="sortorder" select="//query/columns/viewtext1/sorting/@order"/>
														<xsl:with-param name="sortmode" select="//query/columns/viewtext1/sorting/@mode"/>
													</xsl:call-template>
												</td>
												<td style="width:160px; padding-left:10px" class="thcell">
													<xsl:call-template name="sortingcellpage">
														<xsl:with-param name="namefield" select="'byrev'"/>
														<xsl:with-param name="sortorder" select="//query/columns/viewtext1/sorting/@order"/>
														<xsl:with-param name="sortmode" select="//query/columns/viewtext1/sorting/@mode"/>
													</xsl:call-template>
												</td>
												<td style="min-width:280px; padding-left:10px" class="thcell">
													<xsl:call-template name="sortingcellpage">
														<xsl:with-param name="namefield" select="'diff'"/>
							...
							<div id="viewtablediv">
								<div id="tablecontent" style ="top:135px">
									<table class="viewtable" id="viewtable" width="100%">
										<xsl:apply-templates select="//content/invent/entry"/>
									</table>
