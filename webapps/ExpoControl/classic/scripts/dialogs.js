...
function pickListSingleOk(docid){
	...
	if (queryOpt.fieldname == 'executor'){
		...
		$("#frm").append("<input type='hidden' id='controlres1' name='executor' value='1`"+ docid +"`1``'/>");
		$('#intexectable').append("<tr>" +
			"<td style='text-align:center'>1</td>" +
			"<td>"+text+"<input  type='hidden' id='idContrExec1' value='"+docid+"'/></td>" +
			"<td id='controlOffDate1'/>" +
			"<td id='idCorrControlOff1'/>" +
		...
		if(queryOpt.fieldname == 'extexecutor'){
			...
			$("#frm").append("<input type='hidden' id='controlres1' name='extexecutor' value='"+ extexecval +"'/>");
		}else{
	...
	if ($('input[name=chbox]:checked').length > 0){
		...
		if (queryOpt.fieldname == "executor"){
			...
			$('input[name=chbox]:checked').each(function(indx, element){
				...
				if(indx == 0){
					$("#frm").append("<input type='hidden' id='controlres"+indx+"' name='executor' value='1`"+ hidfields[indx].id +"`1``'/>");
				}else{
					$("#frm").append("<input type='hidden' id='controlres"+indx+"' name='executor' value='1`"+ hidfields[indx].id +"`0``'/>");
				}
				$('#intexectable').append("<tr>" +
					"<td style='text-align:center'>"+ (indx+1) +"</td>" +
					"<td>"+hidfields[indx].value+"<input  type='hidden' id='idContrExec"+(indx+1)+"' value='1`"+hidfields[indx].id+"'/></td>" +
...
function dialogBoxStructure(query,isMultiValue, field, form, table) {
	...
	divhtml +="<div class='closeButton'><img style='margin-left:3px; margin-top:1px' src='classic/img/close_black_24.png' onclick='pickListClose();'/>";
	...
	divhtml +="<div  style='display:inline-block; float:right; width:100px; margin-top:20px'></a></div></div>";
	...
	divhtml += "<button class='blue-button' onclick='pickListBtnOk()' style='margin-right:5px'><font class='button_text'>ОК</font></button>" 
	divhtml += "<button class='blue-button' onclick='pickListClose()'><font class='button_text'>"+cancelcaption+"</font></button>";    
...
function closeListRespDoc(docid,doctype,num,page) {
	...
	$("#a"+docid+doctype).attr("href","javascript:openListRespDoc('"+docid+"','"+ doctype+"','"+num+"',1)");
	$("#img"+docid+doctype).attr("src","/SharedResources/img/classic/1/plus.png");
}
...
function selectItemPicklist(element, event){
	...
	if(event.target.nodeName !="INPUT"){
		...
		if($(target_input).attr("checked") == "checked"){
			target_input.removeAttr("checked");
		}else{
