...
function controlOff(pos){
	...
	$("#controlOffDate"+pos).text(actionTime);
	$("#switchControl"+pos).html("<img style='cursor:pointer' title='Поставить на контроль' onclick='javascript:controlOn("+pos+")' src='/SharedResources/img/classic/icons/accept.png'/>");
	execid=$("#idContrExec"+pos).val();
	idCorrControlOff=$("#currentuserid").val();
	$("#controlres"+pos).val( execid +"`"+responsible+"`" +actionTime+"`"+idCorrControlOff);
}
...
function controlOn(pos){
	...
	$("#switchControl"+pos).html("<img style='cursor:pointer' title='Снять с контроля' href='javascript:controlOff("+pos+")' src='/SharedResources/img/classic/icons/eye.png'/>");
...
function submitPhoto(formid, tableID, fieldName) {
	if ($('input[name='+fieldName+']').val() == '' || $('input[name='+fieldName+']').val() == 'undefined') {
		infoDialog(showfilename);
	}else{
		ext = $('input[name='+fieldName+']').val().split(".").pop().toLowerCase();
		if(ext == "jpg" || ext == "jpeg" || ext == "png" || ext == "gif"){
			...
			var frame = createIFrame();
			frame.onSendComplete = function() {
				uploadPhotoComplete(tableID, getIFrameXML(frame));
				$("#loading_attach, #loadingpage, #blockWindow ").remove();
				$("body").css("cursor","default")
			};
			form.attr('target',frame.id);
			...
			alert("Данное поле поддерживает только файлы изображений")
		}
	}
}
...
function uploadComplete(tableID, doc) {
	...
	if (st == 'ok'){
		...
		if(fileName.indexOf(".") != -1){
			...
			fileNameEcr=fileName.replace(/\+/g, "%2b");// убираем знак '+' из ссылок
...
function uploadPhotoComplete(tableID, doc) {
	if (!doc)
		return;
	var xmldoc = doc.documentElement,
	st = xmldoc.getAttribute('status'),
	...
	if (st == 'ok'){
		...
		if(fileName.indexOf(".") != -1){
			...
			fileid=$(doc).find("message[id=4]").text();
			filehash=$(doc).find("message[id=2]").text();
			fileNameEcr=fileName.replace(/\+/g, "%2b");
			$("form[name=upload]").append("<span id='uploadfields'></span>")
			$("#uploadfields").append('<input type="hidden" name="filename" value="'+fileName+'"/><input type="hidden" name="fileid" value="'+fileid+'"/><input type="hidden" name="filehash" value="'+filehash+'"/>')
			$("#photoid").attr("src","Provider?type=getattach&formsesid="+ sesid+"&field=rtfcontent&key="+fileid+"&doctype=896&id=rtfcontent&file="+fileNameEcr)
			$("#progressbar").progressbar("destroy");
			$("#progressstate").css("display","none");
		}else{
			$("#progressbar").progressbar("destroy");
...
function shedule_find_events(){
	setTimeout("find_events()", 1000);
}
var prev_del_notif;
function find_events(){
	$(prev_del_notif).remove()
	$.ajax({
		type: "GET",
		datatype:"XML",
		url: "Provider",
		data: "type=page&id=events",
		cache:false,
		ifModified:true,
		success: function (xml){
			$(prev_del_notif).remove()
			$(xml).find("events").find("entry").each(function(){
				if($("notif_"+$(this).find("id").text()).length == 0){
					if($(this).find("photo").text().length > 0){
						photo ="<img style='border:1px solid #ccc; max-width:190px; max-height:190px;' src='"+$(this).find("photo").text()+"'/>"
					}else{
						photo = ""
					}
					notif_div="<div class='nofication_windows' id='notif_"+$(this).find("id").text()+"' style='width:500px; min-height:130px; z-index:"+$(this).find("id").text()+"; position:absolute; display:none; border: 1px solid #59A3CB; background:#fff'>" +
							"<div id='header_alert' style=' height:30px; color:#fff; background:#4BA6DB; width:500px; border-bottom:1px solid #59A3CB'><center><font style='vertical-align:-20px; font-size:14px;'>Уведомление</font></center><img style='margin-left:3px; margin-top:1px; position:absolute; top:3px; right:3px; cursor:pointer;' src='classic/img/close_black_24.png' onclick='closeNotif("+$(this).find("id").text()+")'/></div>" +
							"<div id='content' style='min-height:100px; width:500px; text-align:center; font-size:15px;'><br/>"+ $(this).find("event_time").text() +" "+$(this).find("action").text()+"<br/><div style='height:8px; display:block; '/><font style='font-size:17px; font-weight:bold'>"+$(this).find("fio").text()+"</font><br/><br/>"+photo+"<br/><br/></div>" +
							"</div>"
					$("body").append(notif_div)
					var w=$(window).width(), 
					h=$(window).height(),
					winH=$('#notif_'+$(this).find("id").text()).height(), 
					winW=$('#notif_'+$(this).find("id").text()).width(),
					scrollA=$("body").scrollTop(), 
					scrollB=$("body").scrollLeft();
					htop=scrollA+((h/2)-(winH/2))
					hleft=scrollB+((w/2)-(winW/2))
					$('#notif_'+$(this).find("id").text()).css({"top":htop, "left":hleft, "display":"block"}) ;
				}
			})
			$(prev_del_notif).remove()
		},
...
function closeNotif(id){
	prev_del_notif = "#notif_"+id;
	$.ajax({
		type: "GET",
		datatype:"XML",
		url: "Provider",
		data: "type=page&id=process_events&event_id="+id,
		cache:false,
		success: function (xml){
			$("#notif_"+id).remove()
		},
