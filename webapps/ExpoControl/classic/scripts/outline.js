...
function mapsReady(){
	...
	if ('contentDocument' in svgobject) {              
		...
		alert(svgdom)
	    $("#path3011", svgdom).attr("fill", "red");  
	}	
    var mySvg = $("#map");
    alert($("#svg2").contentDocument)
    mySvg.addEventListener("load", function() {
        var svg = mySvg.getSVGDocument();
        alert(svg.getElementById("path3011"));
    })*/
	var mySVG = document.getElementById("imap");
	var svgDoc = mySVG.contentDocument;
	
	$(svgDoc.getElementsByClassName("area")).click(function(){ 
		$(".userentry").empty()
		elid= $(this).attr("id");
		$.ajax({
			url: "Provider?type=page&id=getcountbyid&pavid="+$(this).attr("id"),
			dataType:'xml',
			async:'true',
			success: function(xml) {
				count= $(xml).find("view_content").find("query").attr("count")
				name = $(svgDoc.getElementsByClassName(elid)).text()
				tr="";
				$(xml).find("view_content").find("entry").each(function(){
					tr+="<tr class='userentry'><td>"+$(this).find("value").text()+"</td><td>"+$(this).find("userid").text()+"</td></tr>"
				})
				$("#pavinfo_name").text("Название павильона :"+ name)
				$("#pavinfo_count").text("Количество людей в павильоне : "+ count)
				$("#userstable").append(tr);
			}
		});
	})
	$(svgDoc.getElementsByClassName("1-1")).mouseenter(function(){ 
		$(svgDoc.getElementById("1-1")).attr("style"," fill: #9ecaed !important; stroke: #0d77b6 !important;")
	})
	$(svgDoc.getElementsByClassName("1-1")).mouseleave(function(){ 
		$(svgDoc.getElementById("1-1")).attr("style"," fill: #fff ; stroke: #000 ")
	})
	$(svgDoc.getElementsByClassName("1-2")).mouseenter(function(){ 
		$(svgDoc.getElementById("1-2")).attr("style"," fill: #9ecaed !important; stroke: #0d77b6 !important;")
	})
	$(svgDoc.getElementsByClassName("1-2")).mouseleave(function(){ 
		$(svgDoc.getElementById("1-2")).attr("style"," fill: #fff ; stroke: #000 ;")
	})
	$(svgDoc.getElementsByClassName("1-3")).mouseenter(function(){ 
		$(svgDoc.getElementById("1-3")).attr("style"," fill: #9ecaed !important; stroke: #0d77b6 !important;")
	})
	$(svgDoc.getElementsByClassName("1-3")).mouseleave(function(){ 
		$(svgDoc.getElementById("1-3")).attr("style"," fill: #fff ; stroke: #000 ;")
	})
	$(svgDoc.getElementsByClassName("2-1")).mouseenter(function(){ 
		$(svgDoc.getElementById("2-1")).attr("style"," fill: #9ecaed !important; stroke: #0d77b6 !important;")
	})
	$(svgDoc.getElementsByClassName("2-1")).mouseleave(function(){ 
		$(svgDoc.getElementById("2-1")).attr("style"," fill: #fff ; stroke: #000;")
	})
	/*$(svgDoc.getElementsByClassName("area")).mouseleave(function(){ 
		alert(5)
	})
*/	
	//$("path", svgDoc).attr("style","fill:none;stroke:red;stroke-width:0.78732401px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"); 
